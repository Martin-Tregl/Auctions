...
def simulate_multiple(strategies, auction_type='english', count=100):
    ...
    str_money = {strat.name() : 500 for strat in strategies}
    for s in strategies:
...
if __name__ == '__main__':
    ...
    for i in range(0):
        profits_english_i = simulate_multiple(strategies_english, 'english')
        profits_dutch_n = simulate_multiple(strategies_dutch, 'dutch')
        for se in strategies_english:
            profits_english[se.name()] += profits_english_i[se.name()];
        for sd in strategies_dutch:
            profits_dutch[sd.name()] += profits_dutch_n[sd.name()];
